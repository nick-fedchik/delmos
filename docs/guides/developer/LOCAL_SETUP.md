...
> Якщо запуск завершується `permission denied to create database (SQLSTATE 42501)`, це означає, що роль, під якою відбулося з'єднання, не має `CREATEDB`. Таке буває, коли оточення налаштовували не через `make db-test-setup`, а роль названо інакше, ніж ОС-користувач. Перевірте фактичну роль і її права — `psql -tAc "SELECT current_user, rolcreatedb FROM pg_roles WHERE rolname = current_user"` — і за потреби вкажіть потрібну роль явно: `user=<роль>` у DSN.
