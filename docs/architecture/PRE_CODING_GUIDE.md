...
5. **Відтворювана перевірка.** Базові команди build/test/lint/doc links реалізовані в `Makefile` (`make validate`) і виконуються в CI. Тестова БД із початковим і вже мігрованим станом створюється на кожен тест (`make db-test-setup`, `make test-integration`); запуск міграцій під непривілейованою роллю та rollback транзакції лишаються до перевірки на реальній інсталяції.
