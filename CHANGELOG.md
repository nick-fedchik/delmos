...
- Перехід статусу фази проєкту та робочий шлюз ліміту фінансування (SHR-13 критерій 5).
  - `project.TransitionPhase`: `not_started -> active -> completed`. Зворотні й стрибкові переходи заборонені: завершена фаза вже врахована в показниках здобутої цінності.
  - `economics.PhaseGateFacts`: фактичні витрати, ліміт і CPI фази обчислюються **в тій самій транзакції**, що й зміна статусу — між перевіркою ліміту та записом не може вклинитися нове списання.
  - `POST /api/v1/projects/{id}/phases/{phase_key}/transition` під правом `plan.apply`; `422` на вето правила чи неприпустимий перехід (ADR-009 §4).
  - Це замикає ланцюг, залишений відкритим у 1.0.20: тригер `trigger.core.before_phase_transition` і правила `rule.economics.*` тепер мають точку виклику.
  - Тести результативності: перевищення ліміту не лише повертає помилку, а й **залишає фазу відкритою** (транзакція відкочується); витрата рівно на ліміті перехід не блокує; без затвердженого кошторису шлюз не стає глухим блокувальником; ADVISORY_WARNING фіксується в аудиті, але не блокує; чотири варіанти неприпустимих переходів не змінюють статус.
