...
Виправлення дефектів архітектурного аудиту наскрізного шляху проєкту (MEMO-006).

### Виправлено

- `RecordExpense` вимагає відкриту фазу (`ErrPhaseNotOpen`) і, за наявності затвердженого
  кошторису, збіг валюти (`ErrCurrencyMismatch`) — AC підсумовує суми без конвертації, тож
  витрата в іншій валюті мовчки спотворювала б показник.
- `SetLaborRate` стала транзакційною й публікує `economics.inputs_changed` для проєктних
  ставок — зміна ставки тепер запускає перерахунок AC/CPI.
- `TransitionPhase` перевіряє `depends_on`: фаза не активується, доки залежна фаза не
  завершена (`ErrPhaseDependencyNotMet`, CORE-CONTRACT-002 §4.1).
- `ApplyPlan`: повторне застосування тієї самої ревізії ідемпотентне (SWR-46.2); фази й
  віхи, вилучені з нового маніфесту, вилучаються і з проєкції, а вилучення фази з уже
  записаною активністю відхиляється явним конфліктом (`ErrPlanPhaseHasActivity`) замість
  непрозорої помилки СУБД; публікується подія `plan.applied` (CORE-CONTRACT-003 §5).
- `trigger.core.after_economics_changed` підписано на подію `wp.approved` — погодження
  результату впливає на EV через `phase_deliverables`, і без цієї підписки перерахунок не
  запускався.
- **Критичне:** `ReviseWorkProduct` не повертав артефакт у статус `draft` після нової
  ревізії, через що будь-який раз погоджений work product (включно з планом) неможливо
  було подати на повторну рецензію. Виявлено під час написання регресійних тестів для
  цього цикла; виправлено відповідно до власної діаграми життєвого циклу в
  PROJECT_MODEL.md (`Approved → Draft: plan.revise`).

### Примітки

- [MEMO-006](docs/memos/MEMO-006-architecture-audit-2026-09-26.md) фіксує повний перелік
  знахідок аудиту, включно зі свідомо відкладеними задачами (конвертація валют між
  ставками праці й кошторисом, версіонування вхідних даних метрик, приймання майлстоунів
  через `GateDecision`, мультивалютні визначення метрик, вивірка нормативних посилань на
  PMBOK/SWEBOK проти ADR-011).
...
- HTTP-шар проєктної економіки — шість маршрутів запису, яких не існувало:
  `POST .../economics/work-records`, `.../labor-rates`, `.../cost-baselines`,
  `.../cost-baselines/{id}/approve`, `.../expenses`, `.../phase-deliverables`.
  - **Це усуває критичний дефект.** Досі економіка мала повний шар даних і логіки, але нульовий шар API: у працюючій системі неможливо було ввести витрату чи затвердити кошторис, отже шлюз ліміту фінансування ніколи не спрацьовував. Критерій приймання 5 SHR-13 формально не виконувався.
  - Права `timesheet.log`, `economics.manage`, `economics.approve` тепер справді перевіряються — раніше вони були оголошені, але ніде не застосовувалися.
- Наскрізні тести **через HTTP** (`internal/server/economics_http_test.go`): логін, CSRF, реальні маршрути. Корінь дефекту був у тому, що всі тести зверталися до сховища напряму, тож відсутність HTTP-шару лишалася непоміченою.
- Подія `phase.transitioned`: перехід фази тепер публікується в outbox, як і решта команд.

### Виправлено

- **Розподіл обовʼязків для кошторису був заявлений, але не реалізований.** Повідомлення коміта `f10c09e` стверджувало «хто складає кошторис, той його не затверджує», проте `core.cost_baselines` не мала колонки `created_by`, і перевірки самозатвердження не існувало — розділені були лише ключі прав.
  - Міграція `0016`: колонка `created_by` та констрейнт `cost_baselines_segregation_check` рівня рядка. Інваріант тримає СУБД, тож помилка в застосунку його не знімає.
  - `ApproveCostBaseline` повертає `ErrSelfApproval`; HTTP — `422 self_approval`.
- `ENTITY_CATALOG.md`: додано `ReviewRequest`, `ReviewDecision`, `PhaseDeliverable`; опис `WorkRecord` приведено до реалізації (`work_product_id` замість `subject_ref`).
- `SPEC-03`: зафіксовано відхилення реалізації від §1.1 і §1.3 безпосередньо в специфікації, а не лише в журналі змін.

## 1.0.24 — Unreleased

### Змінено

- **Порушення сумісності:** `plan.apply` тепер вимагає погодження ревізії плану (ADR-009 §5). Непогоджений план повертає `422 plan_not_approved`.
  - Перевіряється наявність погодження на **ту саму ревізію**, а не статус артефакта: після першого затвердження план лишається `approved` назавжди, і наступна непогоджена ревізія пройшла б у дію непоміченою.
  - До цього коміта `plan.apply` не був покритий жодним тестом; додано три: відмова без погодження з відкотом, успіх після погодження, відмова новій непогодженій ревізії при `approved` артефакті.

## 1.0.23 — Unreleased

### Додано

- Рішення рецензування та погодження з кворумом і SoD (CORE-CONTRACT-001, ADR-009 §2-4).
  - `project.RecordDecision` для `review` / `approval` / `request_changes`. Перевірки виконуються атомарно в одній транзакції в порядку ADR-009 §4: ідемпотентність → стан → призначення → SoD → актуальність хеша → кворум.
  - Позитивна рецензія свідомо **не** змінює стан артефакта: вона є передумовою погодження, а не погодженням.
  - Одна особа може бути і рецензентом, і погоджувачем лише за наявності **обох** окремих призначень; кожне рішення фіксується окремо.
  - «Потрібні зміни» повертають артефакт у `draft` із мотивованим записом — окремого стану `rejected` немає.
  - `POST .../reviews`, `.../approvals`, `.../request-changes` під правами `wp.review` / `wp.approve` / `wp.request_changes`; коди 403/409/422 за ADR-009 §4.
  - Негативні тести за розділом «Верифікація» ADR-009: самопогодження відхилено навіть при наявному призначенні; непризначена особа не вирішує; рецензент не погоджує без другого призначення; погодження без рецензії відхилено; повтор із тим самим `operation_key` повертає попереднє рішення без другого підпису; рішення поза `in_review` відхилено.
- Наскрізна перевірка здобутої цінності: до погодження EV = `0.00`, після ланцюга submit → review → approve EV = `10000.00`, SPI = `2.0000`. До цього коміта EV завжди дорівнював нулю на реальних даних, бо статус `approved` був недосяжний через API.
